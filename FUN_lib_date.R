#=====================================================
# 函數1：抓一個category所有的文章 v1.2 (08-29)
    ...
    if(len(article.list) != 0) {
      ...
      for(i in 1:nrow(article.df)) {
        ...
        if(str_detect(article, "記者") | str_detect(article, "編譯")) {
          s <- na.omit(c(str_locate(article, "記者")[1], str_locate(article, "編譯")[1])) %>% as.numeric()
          e <- ifelse(na.omit(c(str_locate(article, "╱")[1], str_locate(article, "／")[1])) < s, 
                      na.omit(c(str_locate(article, "╱")[3], str_locate(article, "／")[3])), 
                      na.omit(c(str_locate(article, "╱")[1], str_locate(article, "／")[1])))
          rp_name <- ifelse(len(e) == 0, NA, str_sub(article, s+2, e-1))
          if(!is.na(rp_name)) {
...
#=====================================================
# 函數2：抓一個date所有的文章(包含所有category) v1.0 -> 最早到2005-01-01
#=====================================================
lib.date <- function(...) {
  time.start <- Sys.time()
  len = length
  ...
  for(i in 1:len(categories)) {
    if(i == 1) {
      article.all <- lib.date.category(category = i, ...)
    } else {
      article.all <- rbind.data.frame(article.all, 
                                      lib.date.category(date, i, verbose = F))
    }
  }
  if(verbose) {
    time.interval <- time_length(interval(time.start, Sys.time()), unit = "min") %>% round(2)
